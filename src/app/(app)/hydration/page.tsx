...
export default function Hydration() {
    ...
    const [addedCategories, setAddedCategories] = useState<string[]>(['Water']);
    ...
    useEffect(() => {
        ...
        const stats: HydrationOption[] = hydrationCategories.map(cat => ({
            icon: cat.icon,
            label: cat.label,
            value: cat.label === 'Water' ? "1.50" : undefined,
            unit: 'liters',
            topBg: cat.topBg,
    ...
    return (
        <div className="min-h-screen px-4 py-6 relative font-poppins">
            <div className="text-3xl font-bold mb-4 text-black">today</div>

            <div className="flex flex-wrap gap-4 mb-8">
                {periodOptions.map((opt) => (
                    <button
                        key={opt.label}
                        className={`px-6 py-2 rounded border font-medium text-lg transition-all ${activePeriod === opt.label ? 'bg-black text-white border-black' : 'bg-white text-black border-black/30 hover:border-black'
                            }`}
            ...
            <div className="flex flex-wrap gap-10 mb-4 justify-start">
                {hydrationStats
                    ...
                    .map(({ stat, idx }) => (
                        <div key={idx} className="flex flex-col w-72 h-80 rounded-2xl overflow-hidden shadow-lg cursor-pointer" onClick={() => handleCardClick(idx)}>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-white/90 flex items-center justify-center z-50">
                    <div className="bg-white rounded-2xl p-8 w-[36rem] shadow-2xl border border-gray-100 relative">
                        <button onClick={handleCloseModal} className="absolute top-2 right-4 text-3xl text-black hover:text-red-500 transition-colors">&times;</button>
                        <h2 className="text-xl font-bold mb-6 text-black">Add Hydration Category</h2>
